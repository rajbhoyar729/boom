...
const UploadScreen = ({ navigation }) => {
  ...
  const pickVideo = async () => {
    try {
      ...
      if (!result.canceled) {
        ...
        setThumbnail(result.assets[0].uri) // Set thumbnail state
      }
    } catch (error) {
  ...
  const uploadVideo = async () => {
    // Update validation to check for video, title, and thumbnail
    ...
    try {
      ...
      // Assuming thumbnail is an image file URI, determine its type
